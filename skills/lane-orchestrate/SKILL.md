...
description: Use Lane to run, verify, compare, promote, and clean up isolated AI-agent implementation attempts in one repo without git worktrees or repo copies. Trigger when the user asks Codex to "use lane", try multiple approaches, run several variants, compare designs or fixes, judge attempts, dogfood Lane, or run agents/subagents in parallel lanes.
---

# Lane Orchestrate

Use Lane as the file-versioning layer, not as a planning-file format. The normal flow is prompt -> `lane try` attempts -> `lane check` verification -> `lane compare` decision surface -> promote or resolve selected ops -> discard losing lanes.

## Workflow

1. Confirm the repo has `lane` available with `lane --help`. If the command is missing in a Lane checkout, try `target\debug\lane.exe --help`. If neither works, build or install the CLI before orchestrating attempts.
...
4. Remember that `lane try` reserves fresh `<run>-1`, `<run>-2`, etc. lanes, mounts lane-specific virtual repo views, captures changed bytes back into each lane, leaves the base repo untouched, stores `.lane/runs/<run>.json`, and prints JSON.
...
- Use `lane doctor` when storage health is uncertain. Use `lane gc` only as cleanup for unreferenced blobs after promotion, discard, or failed experimentation.
