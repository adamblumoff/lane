...
fn system() -> String {
    ...
    let _directory = LOCAL_MACHINE
        .open("SOFTWARE\\WOW6432Node\\WinFsp")
        .ok()
        .and_then(|u| u.get_string("InstallDir").ok())
        .expect("WinFsp installation directory not found.");

    // The Windows ARM64 installer provides the runtime DLLs, but not the SDK
    // headers/import libraries in the registry install directory. Use the
    // crate's bundled SDK files while still requiring WinFsp to be installed.
    let project_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap());
    println!(
        "cargo:rustc-link-search={}",
        project_dir.join("winfsp/lib").to_string_lossy()
...
fn copy_winfsp_dll(winfsp_lib: &str) {
    ...
    if let Err(e) = std::fs::create_dir_all(&dll_out_path) {
        panic!(
